...
// 🎨 Animaciones CSS (solo tooltip)
const animations = `
  @keyframes bounce { 0%,100%{ transform:translateY(0);} 50%{transform:translateY(-5px);} }
  @keyframes sleepy { 0%,100%{ transform:rotate(0deg);} 50%{transform:rotate(10deg);} }
  @keyframes burn { 0%{ transform:scale(1) rotate(0deg);} 100%{transform:scale(1.02) rotate(0.3deg);} }
  @keyframes zzz { 0%,100%{ opacity:1; transform:scale(1);} 50%{opacity:0.8; transform:scale(0.98);} }
`;
...
export default function CallsByHourChartBase({ stats }) {
            ...
            <Box sx={{ flex: 1, minHeight: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={enriched} margin={{ top: 30, right: 40, left: 40, bottom: 10 }}>
                  ...
                  <XAxis dataKey="hour" interval={0} angle={-35} textAnchor="end" height={60} tick={{ fontSize: 12 }} />
                  <Tooltip content={<CustomTooltip maxCalls={maxCalls} minCalls={minCalls} />} />
                  <Area type="monotone" dataKey="calls" stroke="#00a1ff" fill="url(#colorCalls)" strokeWidth={2} dot={{ r: 3 }}>
