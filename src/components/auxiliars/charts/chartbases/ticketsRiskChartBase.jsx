...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <Box sx={{ bgcolor: '#fff', border: '1px solid #ccc', borderRadius: 1, p: 1, boxShadow: 1 }}>
      ...
      <Typography variant="body2" color="text.secondary">Calls: {value}</Typography>
    </Box>
  );
};

export default function TicketRiskChartBase({ stats, onCategoryClick }) {
        ...
        <CardContent sx={{ p: 2, flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <Box sx={{ flex: 1, width: '100%', minHeight: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dataRisks} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                <XAxis type="category" dataKey="name" tick={{ fontSize: 12 }} />
                <YAxis type="number" tick={{ fontSize: 12 }} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="value" onClick={handleRiskClick} radius={[10,10,0,0]} cursor="pointer">
                  {dataRisks.map((entry, idx) => <Cell key={`cell-${idx}`} fill={entry.fill} />)}
                  <LabelList dataKey="value" position="top" style={{ fill: '#333', fontSize: 12, fontWeight: 'bold' }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </Box>
